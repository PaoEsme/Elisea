...
// URL de imagen de perfil de placeholder
const userProfileImage = { uri: 'https://via.placeholder.com/80x80.png?text=Perfil' }; 

const PagoExitoso = () => {
  const router = useRouter();

  // Función para obtener y formatear la fecha actual en un formato legible
  const formatearFechaActualLegible = () => {
    const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
    const fecha = new Date().toLocaleDateString('es-ES', options);
    // Capitalizar la primera letra del día para mejorar la presentación (ej. Lunes)
  ...
  const handleVolver = () => {
    // Asumiendo que quieres volver a la página de inicio o a la vista principal
    router.replace('/'); 
  };

  return (
    <View style={styles.container}>
      {/* 🛑 FONDO SUPERIOR MORADO Y HEADER - Diseño de la Imagen 🛑 */}
      <View style={styles.topBackground}>
        ...
        {/* Fecha de Transacción - Actualizada */}
      ...
      <ScrollView contentContainerStyle={styles.main}>
        <View style={styles.datosContainer}>
          ...
          <View style={styles.datoItem}>
            <Text style={styles.datoTitulo}>Lineo de captura</Text>
...
const styles = StyleSheet.create({
  ...
  fechaContainer: {
    alignItems: 'center',
    marginBottom: 10,
    backgroundColor: '#4a1563', // Fondo sutil para la fecha
  ...
  profileContainer: {
    ...
    padding: 3,
    position: 'absolute',
    bottom: -35, // Desplazado hacia abajo para superponer
    elevation: 8,
    shadowColor: '#000',
  ...
  // === MAIN CONTENT (DEBAJO DEL BACKGROUND MORADO) ===
  main: {
    padding: 20,
    paddingTop: 45, // Espacio para la imagen de perfil que se superpone
    maxWidth: 500,
    alignSelf: 'center',
    width: '100%',
  },
